...
\item (a). We have $n_5\mid 6$ and $n_5\equiv 1\pmod{5}$, so $n_5\in\{1,6\}$; also $n_3\mid 10$ and $n_3\equiv 1\pmod{3}$, so $n_3\in\{1,10\}$. If $n_5=6$ and $n_3=10$, then elements of order 5 and 3 alone total $6\cdot 4+10\cdot 2=44>30$, a contradiction. So at least one of $n_5,n_3$ equals 1. If $n_5=6$ then $n_3=1$; let $N$ be the unique (normal) Sylow 3-subgroup. Then $|G/N|=10$; in any group of order 10, the Sylow 5-subgroup is unique and normal (since $n_5\mid 2,\;n_5\equiv 1\pmod{5}\Rightarrow n_5=1$). Its preimage in $G$ is a normal subgroup of order 15. A group of order 15 is cyclic (since $3\nmid 4$), so its unique subgroup of order 5 is characteristic, hence normal in $G$---contradicting $n_5=6$. Therefore $n_5=1$, and (a) is guaranteed. (b) can fail: $n_3=10$ is achievable. (c) is false: $S_3\times\mathbb{Z}/5\mathbb{Z}$ has order 30 and is not abelian.
